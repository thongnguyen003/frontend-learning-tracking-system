...
const StudentModal = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://127.0.0.1:8000/api/add-user', {
        users: [{ name, avatar, role: 'student', class_id: classId }],
      });
      onClose(); // Close modal after successful addition
    } catch (error) {
      setError('Error adding student: ' + error.response.data.message);
    }
  };
